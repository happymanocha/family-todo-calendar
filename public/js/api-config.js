...
class APIClient {
  ...
  // Riddle endpoints (use /v1 prefix)
  async getTodaysRiddle() {
    return await this.request('/v1/riddles/today');
  }

  async markRiddleSolved() {
    return await this.request('/v1/riddles/solve', {
      method: 'POST',
    });
  }

  async getRiddleHint(hintNumber) {
    return await this.request(`/v1/riddles/hint/${hintNumber}`, {
      method: 'GET',
    });
  }
