...
const register = lambdaWrapper(async (event) => {
  ...
  try {
    ...
    if (isCreatingFamily) {
      ...
      if (!body.familyName) {
        return errorResponse(
          'Family name is required when creating family',
          400,
          'VALIDATION_ERROR'
        );
      }
      ...
      return errorResponse(
        'Must specify family code to join or create new family',
        400,
        'VALIDATION_ERROR'
      );
    }
 ...
 */
const updateProfile = lambdaWrapper(async (event) => {
  try {
    const user = getAuthenticatedUser(event);
    const body = parseBody(event.body);
    ...
    if (updates.sex && !['male', 'female', 'other'].includes(updates.sex.toLowerCase())) {
      return errorResponse(
        'Invalid sex value. Must be: male, female, or other',
        400,
        'VALIDATION_ERROR'
      );
    }
    ...
    if (updates.dateOfBirth) {
      ...
      if (isNaN(date.getTime())) {
        return errorResponse('Invalid date format for dateOfBirth', 400, 'VALIDATION_ERROR');
      }
    }
    ...
    const updatedUser = await dynamoService.getUser(user.userId);
    ...
    return errorResponse('Failed to update profile', 500, 'UPDATE_PROFILE_ERROR');
  }
});
