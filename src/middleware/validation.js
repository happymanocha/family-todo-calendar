...
const schemas = {
    ...
    register: Joi.object({
        ...
        familyCode: Joi.string().optional().allow(''),
        familyName: Joi.string().min(2).max(50).when('familyCode', {
            is: Joi.string().length(0),
            then: Joi.required(),
            otherwise: Joi.optional()
        })
            .messages({
                'string.min': 'Family name must be at least 2 characters long',
                'string.max': 'Family name must be less than 50 characters',
                'any.required': 'Family name is required when creating a new family'
            })
    }),
